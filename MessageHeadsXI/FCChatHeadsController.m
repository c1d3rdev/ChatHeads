...
@property (nonatomic, strong) UIView *messagePreviewView;
...
@synthesize DEFAULT_CHAT_HEAD_FRAME, contentView;
...
- (void)setup
{
    DEFAULT_CHAT_HEAD_FRAME = (CGRectMake(SCREEN_BOUNDS.size.width - CHAT_HEAD_DIMENSION - CHAT_HEAD_MARGIN_X,        \
...
- (void)presentChatHeadWithView:(UIView *)view chatID:(NSString *)chatID
{
    ...
    if (!aChatHead.unreadCount) {
        [aChatHead setUnreadCount:1];
    }else{
        [aChatHead setUnreadCount:aChatHead.unreadCount++];
    }
...
- (void)presentMessageFromChatHeads:(NSString *)message{
    if ((![SETTINGS[@"showPreview"] boolValue]) | (message.length == 0) | self.chatHeadsMoving | self.chatHeadsHidden) {
        return;
    }
    
    if (self.messagePreviewView) {
        [self.messagePreviewView removeFromSuperview];
        self.messagePreviewView = nil;
    }

    if (!self.isExpanded)
    {
        FCChatHead *head = self.chatHeads[0];
        
        bool left = NO;
        float x = head.frame.origin.x;
        ...
        UILabel *messageLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 80, 32)];
        messageLabel.backgroundColor = [UIColor respondsToSelector:@selector(systemBackgroundColor)] ? [UIColor performSelector:@selector(systemBackgroundColor)] : UIColor.whiteColor;;
        messageLabel.text = [NSString stringWithFormat:@" %@", message];
        [messageLabel.layer setCornerRadius:8];
        messageLabel.layer.masksToBounds = YES;
        messageLabel.numberOfLines = 1;
        [messageLabel sizeToFit];
        
        CGRect r = messageLabel.frame;
        r.size.width +=8;
        r.size.height = 32;
        messageLabel.frame = r;
        
        if (messageLabel.frame.size.width >= 180) {
            r.size.width = 180;
            messageLabel.frame = r;
        }
        
        if (left) {
            x += CHAT_HEAD_DIMENSION+10;
        }else{
            x -= ((head.frame.size.width/2)+messageLabel.frame.size.width)-16;
        }

        
        
        // drop shadow
        self.messagePreviewView = [[UIView alloc] initWithFrame:CGRectMake(x, head.center.y-16, messageLabel.frame.size.width, 34)];
        self.messagePreviewView.layer.shadowColor = [UIColor blackColor].CGColor;
        self.messagePreviewView.layer.shadowRadius = 4.0;
        self.messagePreviewView.layer.shadowOffset = CGSizeMake(0, 0);
        self.messagePreviewView.layer.shadowOpacity = 0.5;
        [self.messagePreviewView addSubview:messageLabel];
        [self.headSuperView addSubview:self.messagePreviewView];
        
        
        [UIView animateWithDuration:0.3 animations:^{
            self.messagePreviewView.alpha = 1;
            
            [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(hideMessagePreviewAnimated:) object:nil];
            [self performSelector:@selector(hideMessagePreviewAnimated:) withObject:nil afterDelay:3];
        }];
    }
}

-(void)hideMessagePreviewAnimated:(BOOL)animated{
    if (self.messagePreviewView) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [UIView animateWithDuration:0.3 animations:^{
                self.messagePreviewView.alpha = 0;
...
- (void)finishPanEndMotionWithVelocity:(CGPoint)panEndVelocity forChatHead:(FCChatHead *)chatHead
{
    ...
    if (self.isExpanded)
    {
        ...
        if (dismiss) {
            NSLog(@"[MH] expanded and dismiss...");
            [self collapseChatHeads];
//            [self updateChatHeadsLayoutForDraggingChatHead:chatHead toPosition:proposedEndPoint];
            [self moveChatHeadStack:self.chatHeads toLocation:proposedEndPoint animated:YES];
        }
        else
        {
                                    ...
                                    shouldDismiss:(BOOL *)dismiss
{
    ...
    if (self.sinkView.superview)
    {
        ...
        if (self.isExpanded) {
            ...
            CGRect halfScreen = CGRectMake(0, SCREEN_BOUNDS.size.height/2,SCREEN_BOUNDS.size.width,SCREEN_BOUNDS.size.height/2);

            
            if ((proposedFinalY > (SCREEN_BOUNDS.size.height - SCREEN_BOUNDS.size.height/2)) && FCRayIntersectsWithRect(FCRayCreate(initialPosition, CGPointMake(proposedFinalX, proposedFinalY)), halfScreen))
            {
                shouldDismiss = YES;
            }
        }
...
- (void)addRemovalAnimationForChatHead:(FCChatHead *)chatHead
{
    ...
    if ([SETTINGS[@"debug"] boolValue]){
        [self debugAlertWithMessage:[NSString stringWithFormat:@"dismissing head with ID:  %@", chatHead.chatID]];
    }
...
- (void)removeChatHead:(FCChatHead *)chatHead
{
    ...
    if (self.isExpanded)
    {
        ...
        if (wasActive)
        {
            ...
            if (chatHead)
            {
                [self.delegate chatHeadsController:self willPresentPopoverForChatID:chatHead.chatID];
                self.activeChatHead = chatHead;
            }
        }
...
{
    if (self.chatHeadsTransitioning | self.chatHeadsMoving)
    {
        return;
    }
    ...
    if (!self.isExpanded)
    {
        [self hideMessagePreviewAnimated:YES];
        [self setUnreadCount:0 forChatHeadWithChatID:chatHead.chatID];

        if ([self.delegate respondsToSelector:@selector(chatHeadsController:willPresentPopoverForChatID:)])
        {
            [self.delegate chatHeadsController:self willPresentPopoverForChatID:chatHead.chatID];
        }
...
- (void)didLongPressOnChatHead:(FCChatHead *)head{
    [self.delegate didLongPressOnChatHead:head];
}
...
- (void)presentPopover
{
    ...
    r.size.height -= NEGATIVE_SPACE;
    //    frame.size.width = [[UIScreen mainScreen] bounds].size.height-180;
    self.popoverFrame = r;
    
    
    contentView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height)];
    if (self.datasource && [self.datasource respondsToSelector:@selector(chatHeadsController:viewForPopoverForChatHeadWithChatID:)])
    {
        ...
        contentView.frame = self.popoverFrame;
    }
    ...
    if (contentView){
        ...
        CGRect finalFrame;
        finalFrame.origin.x = -NEGATIVE_SPACE;
        finalFrame = self.popoverView.frame;
    }else{
        NSLog(@"[MH] retrying....");
        [self.delegate retryPopOverContextHosting];
        
        UILabel *wontHostLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, contentView.frame.size.width-32, 80)];
        wontHostLabel.numberOfLines = 3;
        [wontHostLabel setTextAlignment:NSTextAlignmentCenter];
        wontHostLabel.text = @"Oops, something went wrong.\nTry that again?";
        wontHostLabel.center = CGPointMake(contentView.center.x, 100);
        [contentView addSubview:wontHostLabel];
        ...
        animation.center = CGPointMake(contentView.center.x, wontHostLabel.frame.origin.y+150);
    ...
    self.popoverView.topMargin = 4.0;
    self.popoverView.cornerRadius = 0.0;
    self.popoverView.delegate = self;
    self.popoverView.backgroundColor = [UIColor respondsToSelector:@selector(systemBackgroundColor)] ? [UIColor performSelector:@selector(systemBackgroundColor)] : UIColor.whiteColor;;
;
...
- (void)dismissPopover
{
    if (self.popoverView)
    {
        ...
        {
            NSLog(@"[MH] calling willDismiss delegate");
            [self.delegate chatHeadsController:self willDismissPopoverForChatID:self.activeChatHead.chatID];
        }
        
        if (self.popoverView.customView.subviews.count > 0) {
            CGRect r = self.popoverView.customView.subviews[0].frame;
            r.origin.y = 0;
            self.popoverView.customView.subviews[0].frame = r;
        }
        ...
        {
            NSLog(@"[MH] calling didDismiss delegate");
...
static BOOL hasGivenFeedback = false;
...
- (void)showSink:(BOOL)animated
{
    ...
    UIImage *sinkCrossImage = [UIImage imageNamed:@"/Library/Application Support/MHXI/dismiss"];
...
-(void)gatherChatHeadsToChatHead:(FCChatHead *)mainHead withProposedEndPoint:(CGPoint)proposedEndPoint{
    for (FCChatHead *head in self.chatHeads) {
        if (head != mainHead) {
            [self updateChatHeadsLayoutForDraggingChatHead:mainHead toPosition:proposedEndPoint];
        }
    }
}
...
-(void)debugAlertWithMessage:(NSString *)message{
    ...
    CPDistributedMessagingCenter *c = [CPDistributedMessagingCenter centerNamed:@"com.c1d3r.messageheadsxi.messagereceived"];
    rocketbootstrap_distributedmessagingcenter_apply(c);
    
    if ([SETTINGS[@"debug"] boolValue]){
        [c sendMessageName:@"debug" userInfo:@{@"message" : message}];
    }
}
