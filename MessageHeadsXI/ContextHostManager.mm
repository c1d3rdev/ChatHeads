...
-(UIView *)hostViewForBundleID:(NSString *)bundleId{
    if (@available(iOS 13, *)){
        [self iOS13HostViewForBundleId:bundleId];
    }else{
        [self hostViewForApplicationWithBundleID:bundleId];
    }
    return nil;
}
...
- (BOOL)isHostViewHosting:(UIView *)hostView {
    if (@available(iOS 13, *)){
        if (hostView){
            NSLog(@"Is HostView hosting: %@", hostView);
            return YES;
        }
    }else{
...
- (UIView *)hostViewForApplicationWithBundleID:(NSString *)bundleID {
    ...
    [self.sceneDelegate contextManager:self scene:parentScene sceneStackDidChange:[self hostViewForApplication:appToHost]];
    return nil;
}
